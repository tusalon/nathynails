...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '5199b9b8-43a4-42bd-9309-3864817efca1'; // ID de Nathy Nails
...
window.getNombreNegocio = async function() {
    ...
    return config?.nombre || 'Nathy Nails';
};
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '59360756';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'nathymanresa@gmail.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Nathy Nails!';
};
...
window.getNtfyTopic = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.ntfy_topic || 'nathynails';
};
...
console.log('✅ config-negocio.js listo para Nathy Nails');
